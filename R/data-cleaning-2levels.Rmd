...
locations_to_merge <- locations_coded %>% 
  ...
  select(project_id, provider_name, location_name = `Location Name`, type, primary_cat, form, region,
         domain, rating, publication_date, report_type = `Report Type`, std_name, level, spin_out)
...
providers <- read_excel("D:/Archive/Socialcare UK/data/2017_rating.xlsx", sheet = 3)
```
...
providers_to_merge <- providers_coded %>% 
  ...
  select(project_id, provider_name, location_name, type, primary_cat, form, region,
         domain, rating, publication_date, report_type = `Report Type`, std_name, level, spin_out)
...
## 4. Drop overlapping provider level data

  # drop the overlapping provider data
combined <- combined_2levels
